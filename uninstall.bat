...
IF EXIST %regasm2FilePathx86% (
	echo Attempting to uninstall 32-bit version for .NET Framework 2.0 or newer
	CALL :com_register %regasm2FilePathx86%, "net20"

	IF %x64% EQU 1 ( 
		echo Attempting to uninstall 64-bit version for .NET Framework 2.0 or newer
		CALL :com_register %regasm2FilePathx64%, "net20" 
	)
) ELSE (

	IF EXIST %regasm4FilePathx86% (
		echo Attempting to uninstall 32-bit version for .NET Framework 4.0 or newer
		CALL :com_register %regasm4FilePathx86%, "net40"
		IF %x64% EQU 1 ( 
			echo Attempting to uninstall 64-bit version for .NET Framework 4.0 or newer
...
:check_admin
    ...
    if %ERRORLEVEL% NEQ 0 (
		echo -----------------------	
		echo UNINSTALLATION ERROR
		echo Uninstall with Administrator Privileges: Right-click %~n0%~x0 and select "Run as administrator"
		echo -----------------------       
		pause
    )
EXIT /B %ERRORLEVEL%